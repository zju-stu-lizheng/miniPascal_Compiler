...
llvm::Value* GenSysWrite(const std::vector<std::shared_ptr<Value_Result> > &args_list, bool new_line) {
...
llvm::Value* GenSysRead(const std::vector<std::shared_ptr<Value_Result> > &args_list, bool new_line){
...
namespace Contents
{
    ...
    llvm::Value* GenSysFunc(std::string id, const std::vector<std::shared_ptr<Value_Result> > &args_list) {
        for (auto &ch: id) ch = tolower(ch);
        if (id == "write") return GenSysWrite(args_list, false);
